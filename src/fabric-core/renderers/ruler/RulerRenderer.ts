// src/fabric-core/renderers/ruler/RulerRenderer.ts
...
export class RulerRenderer {
	public render(
		...
		const offsetX = (-bounds.tl.x + 0.5) * zoom // 我也不知道为什么要加 0.5
		const offsetY = (-bounds.tl.y + 0.5) * zoom // I don't know why to add 0.5 either
		...
		// 水平标尺绘制
		this.renderHorizontalRuler(ctx, viewSize, step, offsetX, zoom)
		
		// 垂直标尺绘制
		this.renderVerticalRuler(ctx, viewSize, step, offsetY, zoom)
